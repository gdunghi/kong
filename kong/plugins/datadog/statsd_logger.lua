...
local function statsd_message(stat, delta, kind, sample_rate, tags)
  local rate = ""
  local str_tags = ""
  ...
  return fmt("kong.%s:%s|%s%s%s", stat, delta, kind, rate, str_tags)
end
...
function statsd_mt:send_statsd(stat, delta, kind, sample_rate, tags)
  local udp_message = statsd_message(stat, delta, kind, sample_rate, tags)
